...
int main(){
	...
	for(p = 0; p < numPlayers; p++){
		...
		for(m = 0; m < testHandSize; m++){
#if(NOISY_TEST == 1)
			printf("\nPlayer: %d, Deck Size: %d, Hand: %d, Smithy Position: %d\n", p, d, s, m);
#endif
			...
			state.handCount[p] = s;
			state.hand[p][m] = smithy; //test different locations of smithy card
			previous = state;
#if(NOISY_TEST == 1)
			printf("\nTesting mixed treasure and action deck\n");
#endif
			zzzsmithycardplay(i, p, m, &state);
#if(NOISY_TEST == 1)
			if(d < 3)
