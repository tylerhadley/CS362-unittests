...
void zzzadventurercardplay(int drawntreasure, int currentPlayer, int cardDrawn,
	...
	while (drawntreasure < 2) {
		...
		treasureDrawn = drawCard(currentPlayer, state);
		if(treasureDrawn == 0){
		...
		}else{drawntreasure = 2;}//end loop if no more cards to draw
	}
	while (z - 1 >= 0) {
	...
	//find adventurer card in hand - less than ideal method used to avoid introducing new function arguments
	int handPos = -1;
	for(int i = 0; i < state->handCount[currentPlayer]; i++){
		if(state->hand[currentPlayer][i] == adventurer)
			handPos = i;
	}
	if(handPos == -1){printf("Error: adventurer not found in hand. It should not have been allowed to be played.\n");}
	//assert(handPos != -1);
...
int zzzminecardplay(int currentPlayer, int handPos, struct gameState* state, int choice1, int choice2){
...
int zzzremodelcardplay(int currentPlayer, int handPos, struct gameState* state, int choice1, int choice2){
      int j = state->hand[currentPlayer][choice1];  //store card we will trash
...
void zzzbaroncardplay(int currentPlayer, int handPos, struct gameState* state, int choice1){
...
int zzzembargocardplay(int currentPlayer, int handPos, struct gameState* state, int choice1){
      //+2 Coins
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
      //player trashes a treasure card, adds a treasure to their hand up to 3 coins higher cost
      zzzminecardplay(currentPlayer, handPos, state, choice1, choice2);
      return 0;
			
    case remodel:
      //player trashes a card, then gains a card up to +2 coins higher cost than trashed card
      zzzremodelcardplay(currentPlayer, handPos, state, choice1, choice2);
      return 0;
    ...
    case baron:
      //+1 buy, player may discard estate to get +4 coins. Otherwise, gain an estate.
      zzzbaroncardplay(currentPlayer, handPos,state, choice1);
      return 0;
    ...
    case embargo: 
      //+2 coins, trash embargo card to place embargo token on supply pile
      zzzembargocardplay(currentPlayer, handPos, state, choice1);
      return 0;
