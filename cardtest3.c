...
int main(){
	...
	for(p = 0; p < numPlayers; p++){
		...
		for(m = 0; m < testHandSize; m++){
#if(NOISY_TEST == 1)
			printf("\nPlayer: %d, Deck Size: %d, Hand: %d, Village Position: %d\n", p, d, s, m);
#endif
			...
			state.handCount[p] = s;
			state.hand[p][m] = village; //test different locations of village card
			previous = state;
#if(NOISY_TEST == 1)
			printf("\nTesting mixed treasure and action deck\n");
#endif
			zzzvilliagecardplay(p, m, &state);
#if(NOISY_TEST == 1)
			if(d < 1)
...
#endif	
#if(NOISY_TEST == 1)
			printf("Actions: %d, Expected: %d\n", state.numActions, previous.numActions +2 );
#endif
#if(ASSERTS == 1)
			assert(state.numActions == previous.numActions + 2);
#endif
